...
{
    qgcApp()->showAppMessage(tr("Lỗi trong quá trình thiết lập liên kết video Solo: %1").arg(socketError));
}
...
{
    if (qIsNaN(vehicle->altitudeRelative()->rawValue().toDouble())) {
        qgcApp()->showAppMessage(tr("Không thể thay đổi độ cao, độ cao của thiết bị bay không được biết."));
        return;
    }

    if (pauseVehicle && !_setFlightModeAndValidate(vehicle, pauseFlightMode())) {
        qgcApp()->showAppMessage(tr("Không thể tạm dừng thiết bị bay."));
        return;
    }
...
bool APMFirmwarePlugin::_guidedModeTakeoff(Vehicle* vehicle, double altitudeRel)
{
    if (!vehicle->multiRotor() && !vehicle->vtol()) {
        qgcApp()->showAppMessage(tr("Thiết bị bay không hỗ trợ cất cánh theo guided "));
        return false;
    }

    double vehicleAltitudeAMSL = vehicle->altitudeAMSL()->rawValue().toDouble();
    if (qIsNaN(vehicleAltitudeAMSL)) {
        qgcApp()->showAppMessage(tr("Không thể cất cánh, vị trí của thiết bị bay không được biết."));
        return false;
    }
    ...
    if (!_setFlightModeAndValidate(vehicle, "Guided")) {
        qgcApp()->showAppMessage(tr("Không thể cất cánh: Thiết bị bay không thể chuyển sang chế độ Guided."));
        return false;
    }

    if (!_armVehicleAndValidate(vehicle)) {
        qgcApp()->showAppMessage(tr("Không thể cất cánh: Thiết bị bay không thể ARM."));
        return false;
    }
...
void APMFirmwarePlugin::startMission(Vehicle* vehicle)
{
    if (vehicle->flying()) {
        ...
        if (!_setFlightModeAndValidate(vehicle, "Auto")) {
            qgcApp()->showAppMessage(tr("Không thể bắt đầu nhiệm vụ: Thiết bị bay không thể chuyển sang chế độ Auto."));
        }
        return;
    }
    ...
    if (!vehicle->armed()) {
        ...
        if (vehicle->fixedWing()) {
            ...
            if (!_setFlightModeAndValidate(vehicle, "Guided")) {
                qgcApp()->showAppMessage(tr("Không thể bắt đầu nhiệm vụ: Thiết bị bay không thể chuyển sang chế độ Guided."));
                return;
            }
        }

        if (!_armVehicleAndValidate(vehicle)) {
            qgcApp()->showAppMessage(tr("Không thể bắt đầu nhiệm vụ: Thiết bị bay không thể ARM."));
            return;
        }
    }
...
QString APMFirmwarePlugin::_getLatestVersionFileUrl(Vehicle* vehicle)
{
    ...
    if (qobject_cast<ArduPlaneFirmwarePlugin*>(vehicle->firmwarePlugin())) {
        ...
        qWarning() << "APMFirmwarePlugin::_getLatestVersionFileUrl Loại firmware thiết bị bay không xác định" << vehicle->vehicleType();
        return QString();
    }
}
...
void APMFirmwarePlugin::_sendGCSMotionReport(Vehicle* vehicle, FollowMe::GCSMotionReport& motionReport, uint8_t estimationCapabilities)
{
    if (!vehicle->homePosition().isValid()) {
        ...
        if (!sentOnce) {
            sentOnce = true;
            qgcApp()->showAppMessage(tr("Theo dõi thất bại: Vị trí Home chưa được đặt. "));
        }
        return;
    }
    ...
    if (!(estimationCapabilities & (FollowMe::POS | FollowMe::VEL | FollowMe::HEADING))) {
        ...
        if (!sentOnce) {
            ...
            qgcApp()->showAppMessage(tr("Theo dõi thất bại: Trạm mặt đất không thể cung cấp thông tin vị trí yêu cầu."));
        }
        return;
    }
