...
QString PX4FirmwarePlugin::flightMode(uint8_t base_mode, uint32_t custom_mode) const
{
    ...
    if (base_mode & MAV_MODE_FLAG_CUSTOM_MODE_ENABLED) {
        ...
        if (!found) {
            ...
            return tr("Không xác định %1:%").arg(base_mode).arg(custom_mode);
        }
    }
...
void PX4FirmwarePlugin::guidedModeGotoLocation(Vehicle* vehicle, const QGeoCoordinate& gotoCoord)
{
    if (qIsNaN(vehicle->altitudeAMSL()->rawValue().toDouble())) {
        qgcApp()->showAppMessage(tr("Không thể đến vị trí, vị trí của thiết bị bay không được biết."));
        return;
    }
...
void PX4FirmwarePlugin::guidedModeChangeAltitude(Vehicle* vehicle, double altitudeChange, bool pauseVehicle)
{
    if (!vehicle->homePosition().isValid()) {
        qgcApp()->showAppMessage(tr("Không thể thay đổi độ cao, vị trí Home không xác định."));
        return;
    }
    if (qIsNaN(vehicle->homePosition().altitude())) {
        qgcApp()->showAppMessage(tr("Không thể thay đổi độ cao, độ cao vị trí Home không xác định."));
        return;
    }
...
void PX4FirmwarePlugin::startMission(Vehicle* vehicle)
{
    if (_setFlightModeAndValidate(vehicle, missionFlightMode())) {
        if (!_armVehicleAndValidate(vehicle)) {
            qgcApp()->showAppMessage(tr("Không thể bắt đầu nhiệm vụ: Thiết bị bay từ chối kích hoạt."));
            return;
        }
    } else {
        qgcApp()->showAppMessage(tr("Không thể bắt đầu nhiệm vụ: Thiết bị bay không chuyển sang chế độ bay %1.").arg(missionFlightMode()));
    }
}
...
void PX4FirmwarePlugin::_handleAutopilotVersion(Vehicle* vehicle, mavlink_message_t* message)
{
    ...
    if (!instanceData->versionNotified) {
        ...
        if (notifyUser) {
            ...
            qgcApp()->showAppMessage(tr("QGroundControl hỗ trợ firmware PX4 Pro phiên bản %1.%2.%3 trở lên. Bạn đang sử dụng phiên bản cũ hơn, điều này có thể dẫn đến kết quả không thể đoán trước. Vui lòng nâng cấp firmware của bạn.").arg(supportedMajorVersion).arg(supportedMinorVersion).arg(supportedPatchVersion));
        }
    }
}
