    ...
    , _runningUnitTests     (unitTesting)
{
    ...
    if (_runningUnitTests) {
        ...
        applicationName = QStringLiteral("%1 CONTROL MASTER").arg("AIPT");
#else
...
bool QGCApplication::_initForNormalAppBoot()
{
    ...
    if (_settingsUpgraded) {
        showAppMessage(QString(tr("Định dạng của cài đặt đã lưu cho %1 đã được thay đổi. "
                    "Các cài đặt đã lưu của bạn đã được đặt lại về mặc định.")).arg(applicationName()));
    }
    ...
    // }
    if (getQGCMapEngine()->wasCacheReset()) {
        showAppMessage(tr("Cơ sở dữ liệu bộ nhớ đệm bản đồ ngoại tuyến đã được nâng cấp. "
                    "Các bộ nhớ đệm bản đồ cũ của bạn đã bị đặt lại."));
    }
    settings.sync();
    return true;
}
...
void QGCApplication::saveTelemetryLogOnMainThread(QString tempLogfile)
{
    ...
    if (_checkTelemetrySavePath(true /* useMessageBox */)) {
        ...
        if (!tempFile.copy(saveFilePath)) {
            ...
            QString error = tr("Không thể lưu nhật ký dữ liệu telemetry. Lỗi sao chép dữ liệu telemetry vào '%1': '%2'.").arg(saveFilePath).arg(tempFile.errorString());
...
bool QGCApplication::_checkTelemetrySavePath(bool /*useMessageBox*/)
{
    ...
    if (saveDirPath.isEmpty()) {
        ...
        QString error = tr("Không thể lưu nhật ký dữ liệu telemetry. Thư mục lưu trữ của ứng dụng chưa được thiết lập.");
        showAppMessage(error);
        return false;
    }
    ...
    if (!saveDir.exists()) {
        ...
        QString error = tr("Không thể lưu nhật ký dữ liệu telemetry. Thư mục lưu trữ dữ liệu telemetry \"%1\" không tồn tại.").arg(saveDirPath);
        showAppMessage(error);
        return false;
    }
...
void QGCApplication::_missingParamsDisplay(void)
{
    if (_missingParams.count()) {
        ...
        showAppMessage(tr("Các tham số bị thiếu trong firmware. Có thể bạn đang chạy một phiên bản firmware không được hỗ trợ hoàn toàn hoặc firmware của bạn có lỗi. Các tham số bị thiếu: %1").arg(params));
        ...
        if (versionFile.open(QIODevice::ReadOnly)) {
            ...
            if (_parseVersionText(version, majorVersion, minorVersion, buildVersion)) {
                if (_majorVersion < majorVersion ||
                    ...
                    showAppMessage(tr("Đã có phiên bản mới hơn của %1. Bạn có thể tải xuống từ aiptgroup.com.").arg(applicationName()).arg(toolbox()->corePlugin()->stableDownloadLocation()), tr("Phiên bản mới có sẵn"));
