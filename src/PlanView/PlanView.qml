...
Item {
    ...
    readonly property string    _armedVehicleUploadPrompt:  qsTr("Thiết bị bay hiện tại đã được ARM. Bạn có muốn tải nhiệm vụ lên thiết bị bay không?")
    ...
    Connections {
        ...
        function onRawValueChanged() {
            if (_visualItems.count > 1) {
                mainWindow.showMessageDialog(qsTr("Áp dụng độ cao mới"),
                                             qsTr("Bạn đã thay đổi độ cao mặc định cho các mục trong nhiệm vụ. Bạn có muốn áp dụng độ cao đó cho tất cả các mục trong nhiệm vụ hiện tại không?"),
    ...
    Component {
        ...
        QGCPopupDialog {
            title:      _planMasterController.managerVehicle.isOfflineEditingVehicle ? qsTr("Chế độ Xem Kế hoạch - Thiết bị bay không kết nối") : qsTr("Chế độ Xem Kế hoạch - Thiết bị bay đã thay đổi")
            ...
            ColumnLayout {
                QGCLabel {
                    ...
                    text:                   _planMasterController.managerVehicle.isOfflineEditingVehicle ?
                                                qsTr("Thiết bị bay liên kết với kế hoạch trong Chế độ Xem Kế hoạch không còn khả dụng. Bạn muốn làm gì với kế hoạch đó?") :
                                                qsTr("Kế hoạch đang được làm việc trong Chế độ Xem Kế hoạch không phải của thiết bị bay hiện tại. Bạn muốn làm gì với kế hoạch đó?")
                }
                ...
                QGCButton {
                    ...
                    text:               _planMasterController.dirty ?
                                            (_planMasterController.managerVehicle.isOfflineEditingVehicle ?
                                                 qsTr("Bỏ qua các thay đổi chưa lưu.") :
                                                 qsTr("Bỏ qua các thay đổi chưa lưu, tải kế hoạch mới từ thiết bị bay.")) :
                                            qsTr("Tải kế hoạch mới từ thiết bị bay.")
                    onClicked: {
                ...
                QGCButton {
                    Layout.fillWidth:   true
                    text:               _planMasterController.managerVehicle.isOfflineEditingVehicle ?
                                            qsTr("Giữ kế hoạch hiện tại.") :
                                            qsTr("Giữ kế hoạch hiện tại, không cập nhật từ thiết bị bay.")
                    onClicked: {
    ...
    PlanMasterController {
        ...
        function waitingOnIncompleteDataMessage(save) {
            var saveOrUpload = save ? qsTr("Lưu") : qsTr("Tải lên")
            mainWindow.showMessageDialog(qsTr("Không thể %1.").arg(saveOrUpload), qsTr("Kế hoạch có các mục chưa hoàn thành. Hoàn thành tất cả các mục và %1 lại.").arg(saveOrUpload))
        }

        function waitingOnTerrainDataMessage(save) {
            var saveOrUpload = save ? qsTr("Lưu") : qsTr("Tải lên")
            mainWindow.showMessageDialog(qsTr("Không thể %1.").arg(saveOrUpload), qsTr("Kế hoạch đang chờ dữ liệu địa hình từ máy chủ để có giá trị độ cao chính xác."))
        }
        ...
        function upload() {
            ...
            switch (_missionController.sendToVehiclePreCheck()) {
                ...
                case MissionController.SendToVehiclePreCheckStateActiveMission:
                    mainWindow.showMessageDialog(qsTr("Gửi đến Thiết Bị Bay"), qsTr("Nhiệm vụ hiện tại phải được tạm dừng trước khi tải lên kế hoạch mới."))
                    break
                case MissionController.SendToVehiclePreCheckStateFirwmareVehicleMismatch:
                    mainWindow.showMessageDialog(qsTr("Tải Lên Kế Hoạch"),
                                                 qsTr("Kế hoạch này được tạo ra cho một loại firmware hoặc thiết bị bay khác so với firmware/loại thiết bị bay của thiết bị bạn đang tải lên. " +
                                                      "Điều này có thể dẫn đến lỗi hoặc hành vi không chính xác. " +
                                                      "Khuyến nghị bạn tạo lại kế hoạch cho firmware/loại thiết bị bay đúng.\n" +
                                                      "Nhấn Ok để tải lên kế hoạch này mặc dù có cảnh báo."),
        ...
        function loadFromSelectedFile() {
            fileDialog.title =          qsTr("Chọn file kế hoạch bay")
        ...
        function saveToSelectedFile() {
            ...
            fileDialog.title =          qsTr("Lưu kế hoạch bay")
            fileDialog.planFiles =      true
    ...
    Item {
            ...
            ToolStripActionList {
                ...
                model: [
                    ...
                    ToolStripAction {
                        ...
                        text:               _editingLayer == _layerRallyPoints ? qsTr("Điểm Rally") : qsTr("Waypoint")
                    ...
                    ToolStripAction {
                        text:       _planMasterController.controllerVehicle.multiRotor ? qsTr("Quay về") : qsTr("Hạ cánh")
        ...
        QGCLabel {
            ...
            text:                       qsTr("Được cung cấp bởi %1").arg(_licenseString)
        }
    ...
    function showLoadFromFileOverwritePrompt(title) {
        mainWindow.showMessageDialog(title,
                                     qsTr("Bạn có thay đổi chưa được lưu/đã gửi. Việc tải từ tệp sẽ mất những thay đổi này. Bạn có chắc chắn muốn tải từ tệp không?"),
    ...
    Component {
        ...
        QGCSimpleMessageDialog {
            title:      qsTr("Tạo Kế Hoạch")
            text:       qsTr("Bạn có chắc chắn muốn xóa kế hoạch hiện tại và tạo một kế hoạch mới không? ")
    ...
    function clearButtonClicked() {
        mainWindow.showMessageDialog(qsTr("Clear"),
                                     qsTr("Bạn có chắc chắn muốn xóa tất cả các mục nhiệm vụ và xóa nhiệm vụ khỏi thiết bị bay không?"),
                                     StandardButton.Yes | StandardButton.Cancel,
    ...
    Component {
        ...
        ColumnLayout {
            ...
            QGCLabel { text: qsTr("Tạo pattern phức tạp:") }
    ...
    function downloadClicked(title) {
        if (_planMasterController.dirty) {
            mainWindow.showMessageDialog(title,
                                         qsTr("Bạn có thay đổi chưa lưu/chưa gửi. Việc tải từ Thiết bị bay sẽ mất các thay đổi này. Bạn có chắc chắn muốn tải từ Thiết bị bay không?"),
                                         StandardButton.Yes | StandardButton.Cancel,
        ...
        ColumnLayout {
            ...
            QGCLabel {
                ...
                text:               globals.activeVehicle ?
                                        qsTr("Bạn có những thay đổi chưa được lưu. Bạn nên tải lên thiết bị bay của mình hoặc lưu vào tệp.") :
            ...
            GridLayout {
                ...
                QGCButton {
                    Layout.columnSpan:  3
                    Layout.fillWidth:   true
                    text:               qsTr("Lưu các waypoints nhiệm vụ dưới dạng KML...")
