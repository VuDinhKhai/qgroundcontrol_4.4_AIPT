...
QVariantList& QGCCorePlugin::analyzePages()
{
    if (!_p->analyzeList.count()) {
        _p->analyzeList.append(QVariant::fromValue(new QmlComponentInfo(tr("Tải nhật ký"),     QUrl::fromUserInput("qrc:/qml/LogDownloadPage.qml"),        QUrl::fromUserInput("qrc:/qmlimages/LogDownloadIcon"))));
#if !defined(__mobile__)
        _p->analyzeList.append(QVariant::fromValue(new QmlComponentInfo(tr("Gắn thẻ địa lý cho hình ảnh"),    QUrl::fromUserInput("qrc:/qml/GeoTagPage.qml"),             QUrl::fromUserInput("qrc:/qmlimages/GeoTagIcon"))));
#endif
        _p->analyzeList.append(QVariant::fromValue(new QmlComponentInfo(tr("Bảng điều khiển MAVLink"),  QUrl::fromUserInput("qrc:/qml/MavlinkConsolePage.qml"),     QUrl::fromUserInput("qrc:/qmlimages/MavlinkConsoleIcon"))));
#if !defined(QGC_DISABLE_MAVLINK_INSPECTOR)
        _p->analyzeList.append(QVariant::fromValue(new QmlComponentInfo(tr("Trình kiểm tra MAVLink"),QUrl::fromUserInput("qrc:/qml/MAVLinkInspectorPage.qml"),   QUrl::fromUserInput("qrc:/qmlimages/MAVLinkInspector"))));
#endif
...
{
    return tr("CẢNH BÁO: Bạn sắp vào Chế độ Nâng cao. " "Việc sử dụng sai có thể khiến thiết bị bay của bạn gặp sự cố, làm mất hiệu lực bảo hành. " "Bạn chỉ nên làm như vậy nếu được hỗ trợ bởi bộ phận hỗ trợ khách hàng. " "Bạn có chắc chắn muốn kích hoạt Chế độ Nâng cao không?");
}
