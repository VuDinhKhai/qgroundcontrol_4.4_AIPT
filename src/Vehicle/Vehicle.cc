...
void Vehicle::_chunkedStatusTextCompleted(uint8_t compId)
{
    ...
    for (const QString& chunk : rgChunks) {
        if (chunk.isEmpty()) {
            ...
            messageText += tr(" ... ", "Chỉ ra phần thiếu trong STATUS_TEXT theo dạng phân mảnh.");
        } else {
            messageText += chunk;
        }
    }
...
void Vehicle::_handleBatteryStatus(mavlink_message_t& message)
{
    ...
    case MAV_BATTERY_CHARGE_STATE_LOW:
        if (batteryStatus.charge_state > _lowestBatteryChargeStateAnnouncedMap[batteryStatus.id]) {
            _lowestBatteryChargeStateAnnouncedMap[batteryStatus.id] = batteryStatus.charge_state;
            batteryMessage = tr("Mức pin %1 thấp");
        }
        break;
    case MAV_BATTERY_CHARGE_STATE_CRITICAL:
        if (batteryStatus.charge_state > _lowestBatteryChargeStateAnnouncedMap[batteryStatus.id]) {
            _lowestBatteryChargeStateAnnouncedMap[batteryStatus.id] = batteryStatus.charge_state;
            batteryMessage = tr("Mức pin %1 là nguy cấp");
        }
        break;
    case MAV_BATTERY_CHARGE_STATE_EMERGENCY:
        if (batteryStatus.charge_state > _lowestBatteryChargeStateAnnouncedMap[batteryStatus.id]) {
            _lowestBatteryChargeStateAnnouncedMap[batteryStatus.id] = batteryStatus.charge_state;
            batteryMessage = tr("Mức pin %1 khẩn cấp");
        }
        break;
    case MAV_BATTERY_CHARGE_STATE_FAILED:
        if (batteryStatus.charge_state > _lowestBatteryChargeStateAnnouncedMap[batteryStatus.id]) {
            _lowestBatteryChargeStateAnnouncedMap[batteryStatus.id] = batteryStatus.charge_state;
            batteryMessage = tr("Pin %1 bị hỏng");
        }
        break;
    case MAV_BATTERY_CHARGE_STATE_UNHEALTHY:
        if (batteryStatus.charge_state > _lowestBatteryChargeStateAnnouncedMap[batteryStatus.id]) {
            _lowestBatteryChargeStateAnnouncedMap[batteryStatus.id] = batteryStatus.charge_state;
            batteryMessage = tr("Pin %1 không tốt");
        }
        break;
    }
...
void Vehicle::_setHomePosition(QGeoCoordinate& homeCoord)
{
    if (homeCoord != _homePosition) {
        ...
        qCDebug(VehicleLog) << "Vị trí Home mới được đặt tại tọa độ: " << homeCoord;
...
{
    Q_UNUSED(errorCode);
    qgcApp()->showAppMessage(tr("Chuyển giao nhiệm vụ thất bại. Lỗi: %1").arg(errorMsg));
}

void Vehicle::_geoFenceManagerError(int errorCode, const QString& errorMsg)
{
    Q_UNUSED(errorCode);
    qgcApp()->showAppMessage(tr("Chuyển giao GeoFence thất bại. Lỗi: %1").arg(errorMsg));
}

void Vehicle::_rallyPointManagerError(int errorCode, const QString& errorMsg)
{
    Q_UNUSED(errorCode);
    qgcApp()->showAppMessage(tr("Chuyển giao Điểm Rally thất bại. Lỗi: %1").arg(errorMsg));
}
...
QString Vehicle::vehicleTypeName() const {
    static QMap<int, QString> typeNames = {
        { MAV_TYPE_GENERIC,         tr("Thiết bị bay không người lái dạng máy bay thông dụng" )},
        ...
        { MAV_TYPE_ONBOARD_CONTROLLER, tr("Bộ điều khiển đồng hành trên thiết bị bay")},
        { MAV_TYPE_VTOL_TAILSITTER_DUOROTOR,   tr("VTOL hai rotor sử dụng các bề mặt điều khiển trong thao tác thẳng đứng, gọi là Tailsitter")},
        { MAV_TYPE_VTOL_TAILSITTER_QUADROTOR,  tr("Quadrotor VTOL sử dụng cấu hình V-shaped trong thao tác thẳng đứng, gọi là Tailsitter")},
        ...
        { MAV_TYPE_GIMBAL,          tr("Gimbal trên thiết bị bay")},
        { MAV_TYPE_ADSB,            tr("Thiết bị ADS-B trên thiết bị bay")},
    };
...
QString Vehicle::_vehicleIdSpeech()
{
    if (_toolbox->multiVehicleManager()->vehicles()->count() > 1) {
        return tr("Thiết bị bay %1 ").arg(id());
    } else {
        return QString();
    }
}
...
{
    _say(tr("Chế độ bay %1 %2 ").arg(_vehicleIdSpeech()).arg(flightMode));
    ...
    float param1, float param2, float param3, float param4, double param5, double param6, float param7)
{
    ...
    if ((targetCompId == MAV_COMP_ID_ALL) || (isMavCommandPending(targetCompId, command) && !_commandCanBeDuplicated(command))) {
        ...
        if (showError) {
            qgcApp()->showAppMessage(tr("Không thể gửi lệnh: %1.").arg(compIdAll ? tr("Lỗi nội bộ - MAV_COMP_ID_ALL không được hỗ trợ") : tr("Đang chờ phản hồi từ lệnh trước.")));
        }
...
void Vehicle::_sendMavCommandFromList(int index)
{
    ...
    if (++_mavCommandList[index].tryCount > commandEntry.maxTries) {
        ...
        if (commandEntry.showError) {
            qgcApp()->showAppMessage(tr("Thiết bị bay không phản hồi lệnh: %1").arg(rawCommandName));
        }
        return;
    }
...
void Vehicle::_handleCommandAck(mavlink_message_t& message)
{
    ...
    if (ack.command == MAV_CMD_FLASH_BOOTLOADER && ack.result == MAV_RESULT_ACCEPTED) {
        qgcApp()->showAppMessage(tr("Bootloader flash thành công"));
    }
#endif
            ...
            if (commandEntry.ackHandlerInfo.resultHandler) {
                ...
                if (commandEntry.showError) {
                    ...
                    case MAV_RESULT_TEMPORARILY_REJECTED:
                        qgcApp()->showAppMessage(tr("Lệnh %1 bị từ chối tạm thời").arg(rawCommandName));
                        break;
                    case MAV_RESULT_DENIED:
                        qgcApp()->showAppMessage(tr("Lệnh %1 bị từ chối").arg(rawCommandName));
                        break;
                    case MAV_RESULT_UNSUPPORTED:
                        qgcApp()->showAppMessage(tr("Lệnh %1 không được hỗ trợ").arg(rawCommandName));
                        break;
                    case MAV_RESULT_FAILED:
                        qgcApp()->showAppMessage(tr("Lệnh %1 thất bại").arg(rawCommandName));
                        break;
                    default:
                        // Do nothing
                        break;
...
void Vehicle::_rebootCommandResultHandler(void* resultHandlerData, int /*compId*/, const mavlink_command_ack_t& ack, MavCmdResultFailureCode_t failureCode)
{
    ...
    if (ack.result != MAV_RESULT_ACCEPTED) {
        ...
        qgcApp()->showAppMessage(tr("Khởi động lại thiết bị bay thất bại"));
    } else {
...
void Vehicle::_doSetHomeTerrainReceived(bool success, QList<double> heights)
{
    if (success) {
        ...
        qgcApp()->showAppMessage(tr("Đặt vị trí Home thất bại, dữ liệu địa hình không có sẵn cho tọa độ đã chọn"));
    }
    // Clean up
    _currentDoSetHomeTerrainAtCoordinateQuery = nullptr;
        ...
        if (now - lastUpdate > 3000) {
            ...
            switch (fenceStatus.breach_type) {
                ...
                case FENCE_BREACH_MINALT:
                    breachTypeStr = tr("Độ cao tối thiểu");
                    break;
                case FENCE_BREACH_MAXALT:
                    breachTypeStr = tr("Độ cao tối đa");
                    break;
                case FENCE_BREACH_BOUNDARY:
                    breachTypeStr = tr("Ranh giới");
                    break;
                default:
                    break;
            }

            qgcApp()->toolbox()->audioOutput()->say(breachTypeStr + " " + tr("Vượt qua ranh giới"));
        }
    } else {
        lastUpdate = now;
    }
