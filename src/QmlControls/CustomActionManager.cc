...
void CustomActionManager::_loadFromJson(QVariant fact)
{
    ...
    {
        qCWarning(GuidedActionsControllerLog) << "Lỗi nội bộ của hành động tùy chỉnh: " << errorString;
        emit actionsChanged();
        return;
    }
    ...
    {
        qCWarning(GuidedActionsControllerLog) << "Hành động tùy chỉnh Tài liệu JSON định dạng không đúng:" << errorString;
        emit actionsChanged();
        return;
    }
    ...
    for (auto actionJson : actionList)
    {
        ...
        {
            qCWarning(GuidedActionsControllerLog) << "Hành động tùy chỉnh JsonValue không phải là một đối tượng: " << actionJson;
            continue;
        }
