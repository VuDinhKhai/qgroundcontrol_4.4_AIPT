...
FactValueGrid::FactValueGrid(QQuickItem *parent)
    : QQuickItem(parent), _columns(new QmlObjectListModel(this))
{
...
FactValueGrid::FactValueGrid(const QString &defaultSettingsGroup)
    : QQuickItem(nullptr), _defaultSettingsGroup(defaultSettingsGroup), _columns(new QmlObjectListModel(this))
{
    _init();
}
...
void FactValueGrid::_loadSettings(void)
{
    ...
    if (version != 1)
    {
        qgcApp()->showAppMessage(tr("Phiên bản cài đặt %1 cho %2 không được hỗ trợ. Cài đặt sẽ được đặt lại về mặc định.").arg(version).arg(_userSettingsGroup), tr("Tải Cài Đặt"));
        settings.remove("");
        qgcApp()->toolbox()->corePlugin()->factValueGridCreateDefaultSettings(_defaultSettingsGroup);
    }
